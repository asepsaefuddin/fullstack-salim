...
export default function DashboardScreen({ route, navigation }) {
  ...
  const [deductHistory, setDeductHistory] = useState([]);
  ...
  const markVisibleTasksAsRead = async (tasks) => {
    try {
      // Only mark currently visible tasks (on current page) as read
      const visibleTasks = tasks.slice(indexOfFirstTask, indexOfLastTask);
      
      const unreadTasks = visibleTasks.filter(task => {
  ...
  const loadData = useCallback(async (isRefresh = false) => {
    ...
    try {
      ...
      // DON'T mark tasks as read on initial load - only when user views them
      // Remove this line: await markVisibleTasksAsRead(newTasks);
  ...
  useFocusEffect(
    useCallback(() => {
      loadData();
      refreshDeductHistory();
      return () => {
        stopRingtone();
        Vibration.cancel();
      };
    }, [loadData, refreshDeductHistory])
  );
  ...
  const handlePageChange = (newPage) => {
    if (newPage > 0 && newPage <= totalPages) {
      ...
      // Mark new visible tasks as read when changing page
      markVisibleTasksAsRead(tasks);
    }
  };
  ...
  const handleDoneTask = async () => {
    ...
    const hasValidDeduct = deductHistory.some(
      h => new Date(h.timestamp) > new Date(selectedTask.assigned_at)
    );
    if (!hasValidDeduct) {
      Alert.alert("Not allowed", "You must perform a Deduct Item action AFTER this task was assigned.");
      return;
    }
  ...
  const openTaskModal = (task) => {
    ...
    // Mark this specific task as read when opened
    if (task.read_by_list && !task.read_by_list.includes(employeeId)) {
      updateTaskReadStatus(task.task_id, employeeId).then(() => {
        setTasks(prev =>
          prev.map(t =>
  ...
  // Replace isToday / refreshHasDeductToday with refreshDeductHistory
  const refreshDeductHistory = useCallback(async () => {
    try {
      const history = await getHistory({ employeeId });
      const deducts = Array.isArray(history)
        ? history.filter(h => h.action === 'deduct')
        : [];
      setDeductHistory(deducts);
    } catch (error) {
  ...
  }, [showTaskModal, refreshDeductHistory]);
          ...
          <View style={styles.section}>
            ...
            {currentTasks.length > 0 ? (
              currentTasks.map(task => {
                ...
                // Check if employee has ANY deduct AFTER task assigned
                const taskAssignedDate = new Date(task.assigned_at);
                const isTaskDeducted = deductHistory.some(h => {
                  const deductDate = new Date(h.timestamp);
                  const isAfter = deductDate > taskAssignedDate;
                  console.log('Deduct History Check:', {
                    taskId: task.task_id,
                    taskTitle: task.title,
                    taskAssigned: taskAssignedDate.toISOString(),
                    deductTime: deductDate.toISOString(),
                    itemName: h.item_name,
                    isAfter,
                    timeDiff: deductDate - taskAssignedDate
                  });
                  return isAfter;
                });
                console.log('Task Badge Debug:', {
                  ...
                  isTaskChecked,
                  isTaskDeducted,
                  totalHistoryCount: deductHistory.length,
                  taskAssignedAt: taskAssignedDate.toISOString()
                });
                return (
                  <TouchableOpacity
                    key={task.task_id}
        ...
        <View style={styles.modalOverlay}>
          <Animated.View style={[styles.modalContent, isDarkMode && styles.modalContentDark, { transform: [{ scale: modalAnim }] }]}>
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                closeTaskModal();
                // Add callback to refresh history when returning
            ...
            {/* Done Task */}
            {(() => {
              const taskAssignedDate = selectedTask ? new Date(selectedTask.assigned_at) : null;
              const hasValidDeduct = selectedTask && taskAssignedDate
                ? deductHistory.some(h => {
                    const deductDate = new Date(h.timestamp);
                    const isValid = deductDate > taskAssignedDate;
                    console.log('Modal Deduct Validation:', {
                      taskId: selectedTask.task_id,
                      taskAssigned: taskAssignedDate.toISOString(),
                      deductTime: deductDate.toISOString(),
                      itemName: h.item_name,
                      isValid,
                      timeDiff: (deductDate - taskAssignedDate) / 1000 / 60
                    });
                    return isValid;
                  })
                : false;
              const disabledDone = !isChecked || !hasValidDeduct || isDone;
              return (
                <>
                  <View style={[styles.doneSection, isDarkMode && styles.doneSectionDark]}>
                    ...
                    <View style={styles.requirementRow}>
                      <Ionicons
                        name={hasValidDeduct ? "checkmark-circle" : "close-circle"}
                        size={20}
                        color={hasValidDeduct ? "#10b981" : "#ef4444"}
                      />
                      <Text style={[styles.requirementText, isDarkMode && styles.requirementTextDark]}>
                        2. Deduct Item AFTER Assigned {hasValidDeduct ? '✓' : '✗'}
                      </Text>
                    </View>
                    {isDone && (
                      <View style={styles.requirementRow}>
