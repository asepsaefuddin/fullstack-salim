...
export const getTasks = async ({ employeeId } = {}) => {
  try {
    ...
    return tasks.map(task => {
      ...
      let deductedBy = [];
      if (Array.isArray(task.deducted_by_list)) {
...
export const canMarkTaskDone = async (taskId, employeeId) => {
  ...
  // Check if employee has done ANY deduct today (not specific to this task)
  const now = new Date();
  ...
  const hasDeductToday = Array.isArray(hist) && hist.length > 0;

  return isChecked && hasDeductToday;
};

export const doneTask = async (taskId, employeeId) => {
  const allowed = await canMarkTaskDone(taskId, employeeId);
  if (!allowed) {
    throw new Error('Task cannot be marked as done. You must check the task and have at least one deduct today.');
  }
...
export const deductTaskItems = async ({ taskId, employeeId, employeeName, items }) => {
  ...
  for (const item of items) {
    ...
    await addHistoryEntry({
      ...
      action: 'deduct',
      // task_id intentionally omitted (column not in history)
    });
  }
