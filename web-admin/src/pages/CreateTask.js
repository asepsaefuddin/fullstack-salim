...
const CreateTask = () => {
  ...
  const taskColumns = [
    ...
    },
    {
      ...
      header: 'Done By',
      render: (t) => {
        const normalized = normalizeEmployeeEntries(t.done_by);
        const doneList = normalized.length ? getEmployeeNames(normalized) : '';
        if (!doneList) return <span className="text-muted">Not done yet</span>;
        return (
          <span>
            {doneList}
            <Badge
              ...
              style={{ borderRadius: 20, padding: '6px 12px', fontWeight: 600 }}
            >
              {t.done_by_count ?? normalized.length}
            </Badge>
          </span>
        );
      },
